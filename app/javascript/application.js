...
document.addEventListener('click', function(event) {
  ...
  // Close  menu if clicked element is not part of the menu or the menu button
  if (!menu.contains(event.target) && !menuBtn.contains(event.target)) {
    menu.classList.remove('active');
  }
});

// Trigger hidden input[type="file"] when separate "Choose file" button is clicked
